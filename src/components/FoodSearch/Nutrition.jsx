...
function Nutrition({foodInfo}){
    ...
    const detail = Object.entries(foodInfo).map( (key) => ({items: key[0], values: key[1]}))
    ...
    const summary = {
    labels: ['size(?)', 'Energy(kcal)', 'carbohydrate(g)', 'total_sugar(g)', 'protein(g)', 'fat(g)', 'Cholesterol(mg)'],

    datasets: [
        {
        label: foodInfo.name,
        fill: true,
        data: [foodInfo.size, foodInfo.kcal, foodInfo.carbohydrate, foodInfo.total_sugar, foodInfo.protein, foodInfo.fat, foodInfo.Cholesterol],
