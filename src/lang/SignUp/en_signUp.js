const EN_SIGN_UP = {
  signUpTitle: 'Sign Up',
  signUpEmail: 'E-mail',
  signUpPassword: 'Password',
  signUpVerifyPassword: 'Verify Password',
  signUpNickname: 'Nickname',
  signUpCountry: 'Country',
  signUpCountryKorea: 'Korea',
  signUpCountryOthers: 'Others',
  signUpSubmit: 'Sign Up',
  // Validation
  signUpRequiredErr: 'Enter an {{what}}.',
  signUpEmailMaxErr: 'Your e-mail must be max 255 characters.',
  signUpEmailFormErr: 'Invalid email format.',
  signUpNicknameMaxErr: 'Your nickname must be max 20 characters.',
  signUpPasswordFormErr:
    'Your password must be 8-16 characters, and include at least one uppercase/lowcase letter, a number, and a symbol(~!@#$%*).',
  signUpPasswordVerifyErr: 'The password you entered does not match.',
  signUpCountryRequiredErr: 'Choose a country.',
  // Authentication
  signUpSameEmailErr: 'A user is already registered with this e-mail address',
  signUpServerErr: 'Server connection is unstable.',
};

export default EN_SIGN_UP;
